...
- `src/components/glowbraid/` — the React shell: `GlowbraidStudio.tsx` owns all state; panels, transport bar, canvas interaction, and the animation loop live alongside it.

Saved projects (`localStorage`, key `glowbraid.project`) store only seeds and settings — geometry is regenerated deterministically on load, not persisted directly.
